...
class TestContato(TestCase):
    def setUp(self):
        ...
        self.tec = Contato.objects.create(nome="Maria", funcao="Chefe", telefone="8499343434", email="asdasda@adsadas.com",  licitacaoId=self.licita)
        self.valid_payload = { "nome" : "João",
                               "funcao" : "Analista",
                               "telefone" : "8499333434",
                               "email" : "asdasdad@aasdsad.com",
                               "licitacaoId": 1 }
        self.url = reverse('contato-list')
...
class TestContatoPost(TestContato):
    def test_create_valid_contato(self):
        self._super_auth()
        response = self.client.post(
            reverse('contato-list'),
            data=json.dumps(self.valid_payload),
            content_type='application/json'
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

class TestContatoGet(TestContato):
    def test_authorized_get_contato(self):
        self._super_auth()
        response = self.client.get(self.url)
        expected_data = ContatoSerializer(Contato.objects.all(), many=True).data
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data, expected_data)
        self.assertEqual(Contato.objects.count(), 1)


    def test_unauthorized_get_contato(self):
        response = self.client.get(self.url, format='json')
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

class TestContatoPut(TestContato):
    def test_update_valid_contato(self):
        self._super_auth()
        response = self.client.put(
            reverse('contato-detail', kwargs={'pk': self.tec.pk}),
            data=json.dumps(self.valid_payload),
            content_type='application/json'
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        response2 = self.client.get(
            reverse('contato-detail', kwargs={'pk': self.tec.pk}))
        self.assertEqual(response2.data['nome'], self.valid_payload['nome'])

class TesteTipoDelete(TestContato):
    def test_delete_valid_contato(self):
        self._super_auth()
